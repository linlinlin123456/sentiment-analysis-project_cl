...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the isolated SST -> MLM -> TweetEval pipeline with SST downsampling and optional LangChain refinement."
    )
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Stratified keep ratio for SST Stage 1 training data. Use values below 1.0 to reduce SST influence.",
    )
    parser.add_argument(
...
def detect_runtime(args: argparse.Namespace) -> RuntimeConfig:
    ...
    print(f"  Stage 1 downsample ratio: {args.stage1_downsample_ratio}")
...
def stratified_downsample_dataset(
    ...
    """Stratified downsample for labeled classification data."""
    ...
    for _, group in df.groupby("label", sort=True):
        target_size = max(1, int(round(len(group) * keep_ratio)))
        target_size = min(target_size, len(group))
    ...
    print(
        f"{dataset_name} downsampled with keep_ratio={keep_ratio}: "
...
def run_stage1(
    ...
    dataset["train"] = stratified_downsample_dataset(
        ...
        seed=runtime.seed,
        dataset_name="Stage 1 SST train",
    )
